...
| Column | Type | Notes |
|---|---|---|
| `id` | varchar PK | grid reference `A1`–`I9` (81 areas) |
| `number` | int | |
| `name`, `short_name` | varchar | "Walled City (F4)", "Walled City" |
| `boundary` | JSON | GeoJSON square (~2.5 km) |
...
|---|---|
| `id` | int PK |
| `key` | de-duplication key, e.g. `F4:disruption` (indexed) |
...
1. creates tables (`Base.metadata.create_all`) and (re)writes the 81 areas if the layout changed,
2. checks the history: if it's missing, older than a day, lacks the replay archive, or was made
   for a different area layout, it regenerates it (≈ 20 s, once; later starts reuse it):
   - every **5 minutes for 3 days**: all metrics for all 81 areas (~630k rows, baseline learning),
...
-- Walled City (F4) rainfall over the last few minutes
SELECT ts, value FROM civic_readings
WHERE zone_id = 'F4' AND metric = 'rain_mm_h' AND is_history = 0 ORDER BY ts DESC LIMIT 20;

-- Reports by category in the recorded storm
SELECT category, COUNT(*) FROM incidents
WHERE is_history = 1 AND zone_id = 'E6' GROUP BY category ORDER BY 2 DESC;
```
