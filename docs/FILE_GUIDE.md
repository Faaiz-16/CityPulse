...
| `llm.py` | Optional LLM call | System prompt with grounding rules; `messages.parse` with `SummaryOut` schema; returns `None` on any failure |
...
| `history.py` | Synthetic multi-day history | `generate_history()` every 5 min for 3 days, plus a 1-minute archive around a recorded storm (yesterday evening, over Tonk Phatak C3-8 and Malviya Nagar C4-6) with realistic minute-scale lags |
...
| `map/CityMap.tsx` | Leaflet map of Jaipur on a canvas: amber/red tint only on unusual blocks (grid lines, A-E/1-5 references and district names are behind `SHOW_GRID`, off), one label per hotspot, rain, congestion on real roads, incident clusters, air, sensors; click/hover by grid arithmetic; layers redraw only when their content changes |
