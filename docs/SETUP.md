...
Expected: `121 passed` in about 2–3 minutes. Tests use a temporary database and a simulated clock,
so they are deterministic; the first test builds history for the 81 Jaipur areas (~20 s).

The first backend start does the same (~20 s, once); later starts reuse the stored history.
