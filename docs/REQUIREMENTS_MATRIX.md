...
| 6 | *Optional:* threshold alerting | Monitoring agent opens/resolves alerts with observed vs possible-link wording | M12 `backend/app/agent/` | `test_ai_agent_simulation.py` | Alert appears when the Walled City (F4) goes RED | ✅ Done |
...
| Understandable in ~10 seconds | Default view = Jaipur map in a 9 × 9 grid; only unusual areas are tinted, one label per hotspot, ≤ 4 grouped alerts; area panel answers in 10 seconds with "Explain in detail" behind a button | M9 | Manual 10-second check | Opening screen | ✅ Done |
