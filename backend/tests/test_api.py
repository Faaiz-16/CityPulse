...
def test_zone_endpoints(client):
    ...
    detail = client.get("/api/zones/F4").json()
    assert detail["zone"]["id"] == "F4" and "series" in detail and "explanation" in detail
    missing = client.get("/api/zones/Z9")
    assert missing.status_code == 404 and "Unknown area" in missing.json()["detail"]
...
def test_readings_endpoint(client):
    rows = client.get("/api/readings", params={"zone_id": "E4", "metric": "congestion_pct"}).json()
    assert rows and {"ts", "value", "data_status"} <= set(rows[0])


def test_invalid_simulation_requests_get_clear_errors(client):
    bad_event = client.post("/api/simulation/event", json={"event": "meteor", "zone_id": "F4"})
...
def test_simulation_endpoints(client):
    assert client.post("/api/simulation/event", json={"event": "heavy_rain", "zone_id": "F4"}).json()["ok"]
    ...
    assert client.get("/api/simulation/status").json()["scenario"]["focus_zone"] == "F4"
