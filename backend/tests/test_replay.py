...
def test_replay_finds_the_recorded_event_and_builds_minute_frames(replay):
    meta = replay.meta()
    assert meta["available"] and meta["focus_zone"] == "E6"
    frames = meta["frames"]
...
def test_replay_story_calm_then_disruption_then_recovery(replay):
    ...
    assert frames[0]["statuses"]["E6"] == "GREEN"  # calm before the storm
    assert any(f["statuses"]["E6"] == "RED" for f in frames)
    assert any(f["statuses"]["F7"] == "RED" for f in frames)  # the storm also reached Malviya Nagar
    assert frames[-1]["statuses"]["E6"] == "GREEN"  # recovery
    assert all(f["statuses"]["A1"] == "GREEN" for f in frames)  # areas far from the storm stay calm
...
def test_replay_frame_uses_same_engine_and_agent(replay):
    meta = replay.meta()
    red = next(f["i"] for f in meta["frames"] if f["statuses"]["E6"] == "RED")
    state = replay.frame(red + 3).state
    assert state.mode == "replay" and state.replay_time == state.generated_at
    e6 = next(z for z in state.zones if z.id == "E6")
    assert any(r.rule_id == "rain_traffic" for r in e6.relationships)
...
def test_replay_zone_detail_has_minute_series(replay):
    detail = replay.zone_detail(40, "E6")
...
def test_storm_archive_does_not_distort_live_baselines(pipeline):
    ...
    assert pipeline.baselines.expected_incidents("E6", "waterlogging_reports", evening, 10) < 1
    assert pipeline.baselines.continuous("E6", "rain_mm_h", evening).median == 0
...
def test_replay_api():
    with TestClient(create_app(), raise_server_exceptions=False) as client:
        ...
        assert frame["mode"] == "replay" and len(frame["zones"]) == 81
        assert client.get("/api/replay/frames/30/zones/E6").json()["zone"]["id"] == "E6"
